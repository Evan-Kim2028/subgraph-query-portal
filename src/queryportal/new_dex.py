...
#################################################################################################################################
    ...
    @timeit
    @df_describe
    def generic_query(self, entity:str = None, subgraph_name: str = None, query_paths: list[str] = None, query_size=5, filter_dict={}, saved_file_name=None, add_endpoint_col=True) -> pl.DataFrame:
        """
        This method constructs a query from the Swap entity. It takes in query parameters as inputs and returns a 
        Polars DataFrame of the query results. The method also adds a datetime column to the swaps entity by converting the 
        timestamp column to datetime format using a synthetic field. The `add_endpoint_col` parameter is used to add the 
        ...
        # first we process the subgraph_name for single/multi value inputs.
        if subgraph_name == None:
            try:
                assert len(self.subject.subgraphs) == 1 # check that there is only 1 subgraph endpoint loaded
                single_key = list(self.subject.subgraphs.keys())
            ...
            # load subgraph from the key name
            sg_key = self.subject.subgraphs[subgraph_name]
            print(f'Subgraph endpoint: {subgraph_name} with type {type(sg_key)}')
